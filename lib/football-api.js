...
class FootballAPI {
  ...
  async getTodayMatches() {
    try {
      ...
      const topLeagues = [39, 140, 135, 78, 61, 2]; // API-Football league IDs
      ...
      for (const leagueId of topLeagues) {
        try {
          ...
          if (error.response?.status === 403 || error.response?.data?.message?.includes('not subscribed')) {
            console.log('🚨 API-Football key is not subscribed. Using fallback data.');
          } else if (error.response?.status === 429) {
            console.log('⚠️ API-Football rate limit exceeded. Using fallback data.');
          }
        }
      }
