...
class FootballAPI {
  ...
  // 🌟 Filter matches to only include popular teams OR popular leagues
  filterPopularTeamMatches(matches) {
    ...
    return matches.filter(match => {
      ...
      // Include match if either team is in our popular teams list OR from popular league
      ...
      // More permissive: include if from popular league (even without popular teams)
      const shouldInclude = hasPopularTeam || isPopularLeague;
      
      if (shouldInclude) {
        console.log(`✨ Including match: ${match.teams?.home?.name} vs ${match.teams?.away?.name} (League: ${match.league?.name})`);
      }
