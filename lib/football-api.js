...
class FootballAPI {
  constructor() {
    ...
    // 🌟 TOP 50 POPULAR TEAMS (based on global following)
    this.popularTeams = [
  ...
  // 🌟 Filter matches to only include popular teams
  filterPopularTeamMatches(matches) {
    ...
    return matches.filter(match => {
      ...
      // Include match if either team is in our popular teams list
      const hasPopularTeam = this.popularTeams.includes(homeTeamId) || 
                            this.popularTeams.includes(awayTeamId);
      
      if (hasPopularTeam) {
        console.log(`✨ Popular match: ${match.teams?.home?.name} vs ${match.teams?.away?.name}`);
      }
      
      return hasPopularTeam;
    });
  }
  ...
  async getLiveMatches() {
    ...
    try {
      ...
      // Filter live matches to only popular leagues and teams
      const filteredFixtures = fixtures.filter(fixture => {
        const leagueId = fixture.league.id;
        const homeTeamId = fixture.teams.home.id;
        const awayTeamId = fixture.teams.away.id;
        
        // Only include if from popular league AND has at least one popular team
        ...
        return isPopularLeague && hasPopularTeam;
      });
  ...
  async getUpcomingMatches() {
    ...
    return await this.retryApiCall(async () => {
      ...
      // Filter to only popular leagues and teams first
