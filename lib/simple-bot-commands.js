...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async showAnalyticsReport(chatId) {
    try {
      ...
      // Build analytics message in English
      ...
      message += `📝 Messages Sent: ${overview.totalMessagesPosted}\n`;
      ...
      message += `🎁 Promo: ${dailyStats.promos.posted} (${dailyStats.promos.clicks} clicks)\n\n`;
      
      // Top performing content
      if (overview.topPerformingContent && overview.topPerformingContent.length > 0) {
        message += `🏆 <b>Top Performing Content:</b>\n`;
      ...
      if (error.code === 'ECONNREFUSED' || error.response?.status === 400) {
        errorMessage += 'System is not currently running.\n';
