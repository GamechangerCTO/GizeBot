...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async showAnalyticsReport(chatId) {
    try {
      ...
      if (!data.success) {
        await this.bot.sendMessage(chatId, 
          '⚠️ <b>אנליטיקות לא זמינות</b>\n\n' +
          'המערכת צריכה להיות פעילה כדי לראות נתוני אנליטיקות.\n' +
          'התחל את המערכת עם /api/start ונסה שוב.',
          { parse_mode: 'HTML' }
        );
        return;
      }
      ...
      let message = `📊 <b>דוח אנליטיקות GizeBets</b> 📊\n\n`;
      ...
      const keyboard = {
        inline_keyboard: [
          [
            { text: '🔄 רענן נתונים', callback_data: 'cmd_analytics' },
            { text: '📈 סטטוס מערכת', callback_data: 'cmd_status' }
          ],
          [
      ...
      let errorMessage = '❌ <b>שגיאה בקבלת נתוני אנליטיקות</b>\n\n';
      
      if (error.code === 'ECONNREFUSED' || error.response?.status === 400) {
        errorMessage += 'המערכת לא פעילה כרגע.\n';
        errorMessage += 'התחל את המערכת עם:\n';
