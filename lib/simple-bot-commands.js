...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async startButtonsWizard(chatId, messageId) {
    ...
    await setState(chatId, { type: 'buttons', step: 1, data: {} });
    await this.bot.editMessageText('🧩 Step 1/4: Enter text for Button 1', { chat_id: chatId, message_id: messageId });
  }

  async startCouponWizard(chatId, messageId) {
    const { setState } = require('./wizard-state');
    await setState(chatId, { type: 'coupon', step: 1, data: {} });
    await this.bot.editMessageText('🎟️ Step 1/3: Enter coupon code', { chat_id: chatId, message_id: messageId });
  }
