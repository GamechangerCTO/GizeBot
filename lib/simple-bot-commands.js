...
class SimpleBotCommands {
  constructor() {
    ...
    // Admin users - add your Telegram user IDs here
    this.adminUsers = [
      2024477887, // Test admin
...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async executePredictions(chatId) {
    try {
      ...
      if (matches.length === 0) {
        return await this.bot.sendMessage(chatId, '❌ No matches found for predictions');
      }

      // Generate and send predictions
      const predictions = await contentGenerator.generatePredictions(matches);
      await telegram.sendPredictions(predictions, matches);
  ...
  async executePromo(chatId) {
    try {
      ...
      // Send promo using the existing system
  ...
  async executeResults(chatId) {
    try {
      ...
      if (results.length === 0) {
        return await this.bot.sendMessage(chatId, '❌ No results found');
      }

      // Generate and send results
      const resultsContent = await contentGenerator.generateResults(results);
      await telegram.sendResults(resultsContent, results);

      await this.bot.sendMessage(chatId, 
        `✅ <b>Results sent successfully!</b>\n\n` +
        `📊 <b>Matches:</b> ${results.length}\n` +
        `📝 <b>Channel:</b> @gizebetgames\n` +
        `⏰ <b>Time:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })}`,
        { parse_mode: 'HTML' }
      );
