...
class SimpleBotCommands {
  ...
  async showPersonalSubscribers(chatId) {
    try {
      ...
      // 1) Pull recent clicks on "Get Personal Coupons" from button_analytics (last 30 days)
      ...
      const { data: clickRows, error: clickErr } = await supabase
        ...
        .or('button_text.ilike.%Personal Coupons%,url_clicked.ilike.%t.me/Africansportbot%');
      if (clickErr) throw clickErr;

      // Normalize to numeric IDs and dedupe
      const clickedUserIds = [...new Set((clickRows || [])
        .map(r => Number(String(r.user_id).replace(/[^0-9-]/g, '')))
        .filter(v => Number.isFinite(v) && v > 0))];

      // 2) Cross with interactions consent (users who completed opt-in)
      const { data: consentRows } = await supabase
        .from('interactions')
        .select('user_id, ts')
        .eq('type', 'consent')
        .gte('ts', since30d);
      const consentUserIds = new Set((consentRows || []).map(r => Number(r.user_id)));

      // 3) Fetch user details
      const targetIds = clickedUserIds.length ? clickedUserIds : Array.from(consentUserIds);
      ...
      const header = `👥 <b>Personal Subscribers</b>  |  last 30 days\n` +
