...
class SimpleBotCommands {
  ...
  async showPersonalSubscribers(chatId) {
    try {
      ...
      // We record consent via user-analytics and could also record interactions
      // Prefer pulling from interactions table where type='consent' OR via user_metrics high-consent users
      const { data: consentEvents, error } = await supabase
        .from('interactions')
        .select('user_id, context, ts')
        .eq('type', 'consent')
        .order('ts', { ascending: false })
        .limit(100);

      if (error) throw error;
      const userIds = [...new Set((consentEvents || []).map(r => r.user_id))];
      let users = [];
      if (userIds.length) {
      ...
      if (users.length === 0) {
        return await this.bot.sendMessage(chatId, '👥 No personal subscribers found yet.');
      }

      // Build compact list
      const lines = users.slice(0, 50).map((u, idx) => {
        ...
        return `${idx + 1}. ${name} (${u.user_id})${seen ? ` — last seen: ${seen}` : ''}`;
      });

      const header = `👥 <b>Personal Subscribers</b> (unique: ${users.length})\n`;
      const body = lines.join('\n');
      await this.bot.sendMessage(chatId, header + body, { parse_mode: 'HTML' });
    } catch (e) {
...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async handleButtonsConfig(msg) {
    ...
    const template = `🧩 Send buttons JSON (1-3 buttons), or simple format.\n\n` +
      `JSON example:\n` +
      `[{"text":"⚽ Football","url":"https://gizebets.et/league?sportId=0"},{"text":"🔴 Live","url":"https://gizebets.et/live"},{"text":"🎁 Coupon","url":"https://gizebets.et/promo-campaigns"}]\n\n` +
      `Simple format (semicolon separated rows, comma separated buttons):\n` +
      `Football=https://gizebets.et/league?sportId=0, Live=https://gizebets.et/live, Coupon=https://gizebets.et/promo-campaigns\n\n` +
  ...
  async setBotCommands() {
    try {
      const commands = [
        ...
        { command: 'buttons', description: '🧩 Configure Buttons' },
        { command: 'coupon', description: '🎟️ Configure Coupon' }
      ];
    ...
    try {
      ...
      await this.bot.sendMessage(chatId,
        ...
        { 
          ...
          reply_markup: {
            inline_keyboard: [[
              { text: '🔄 Restart Bot', url: 'https://gize-bot.vercel.app/api/fix-webhook' }
            ]]
          }
        }
      );
