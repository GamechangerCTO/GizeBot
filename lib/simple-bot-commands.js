...
👤 <b>Admin:</b> ${new Date().toLocaleString('en-US', { timeZone: 'Africa/Addis_Ababa' })} (ET)`;
  ...
  async executeTodayHype(chatId) {
    try {
      ...
      const imageBuffer = await imageGen.generateTodayHypeImage(matches);

      if (imageBuffer) {
        await telegram.bot.sendPhoto(telegram.channelId, imageBuffer, {
          caption: content,
          parse_mode: 'HTML',
          reply_markup: { inline_keyboard: keyboard }
        });
      } else {
        await telegram.bot.sendMessage(telegram.channelId, content, {
          parse_mode: 'HTML',
