...
const GizeBetsScheduler = require('./scheduler');
...
class GizeBotCommands {
  constructor() {
    ...
    console.log('🔧 Bot constructor - Token exists:', !!process.env.TELEGRAM_BOT_TOKEN);
  ...
  async setupBotCommands() {
    try {
      ...
      const commands = [
        ...
        {
          command: 'force_stop',
          description: '🔥 Force stop all bot instances'
        },
        {
          command: 'health',
  ...
  setupSendPromoCommand() {
    this.bot.onText(/\/sendpromo(?:\s+(.+))?/, async (msg, match) => {
      ...
      try {
        ...
        // Call the manual promo API with authentication
  ...
  setupSendBonusCommand() {
    this.bot.onText(/\/sendbonus\s+(\w+)\s+"([^"]+)"/, async (msg, match) => {
      ...
      try {
        ...
        // Call the manual bonus API with authentication
  ...
  setupPredictionsCommand() {
    this.bot.onText(/\/predictions/, async (msg) => {
      ...
      try {
        ...
        // Call the manual predictions API with authentication
  ...
  setupResultsCommand() {
    this.bot.onText(/\/results/, async (msg) => {
      ...
      try {
        ...
        // Call the manual results API with authentication
      ...
      try {
        ...
        try {
          ...
          const summary = {
            ...
            readyToSend: upcomingMatches.length,
            scheduled: 0,
            overdue: 0
          };
          
          const nextActions = [
            'Check live matches for in-play predictions',
            'Monitor upcoming matches for pre-match content',
  ...
  async handleManualPromo(chatId, messageId) {
    try {
      console.log(`📱 handleManualPromo called: chatId=${chatId}, messageId=${messageId}`);
      await this.bot.editMessageText(
        '🎁 <b>MANUAL PROMO</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n' +
      ...
      console.log('✅ handleManualPromo completed successfully');
    } catch (error) {
      console.error('❌ Error in handleManualPromo:', error.message);
    }
  }
  ...
  // 🔘 Setup callback query handlers with PRODUCTION-READY response
  setupCallbackHandlers() {
    this.bot.on('callback_query', async (callbackQuery) => {
      ...
      try {
        // 🛡️ Check if callback query is too old (prevent timeout errors)
        if (queryAge > 50000) { // 50 seconds safety margin - increased for better UX
        ...
        // ⚡ Answer callback query immediately with timeout protection
        const answerPromise = this.bot.answerCallbackQuery(callbackQuery.id, {
          text: 'Processing...',
          show_alert: false
        });
        
        // Race condition protection - timeout after 20 seconds
        await Promise.race([
          answerPromise,
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Answer timeout')), 20000)
          )
        ]).catch(err => {
          console.log(`⚠️ Failed to answer callback query: ${err.message}`);
        });

        console.log(`🎯 About to process switch case for action: "${action}"`);
        ...
        try {
          ...
          case 'manual_promo':
            console.log('🎯 Executing manual_promo case');
            await this.handleManualPromo(chatId, messageId);
            console.log('✅ manual_promo case completed');
            break;
          ...
          case 'confirm_send_promo':
            console.log('🎯 Executing confirm_send_promo case');
          ...
          console.error('❌ Error in callback switch statement:', switchError);
          await this.bot.editMessageText('❌ Error processing request. Please try again.', {
            chat_id: chatId,
            message_id: messageId
          });
        }
        ...
        // 🛡️ Production-ready error response with timeout protection
        try {
          const errorQueryAge = Date.now() - (callbackQuery.message.date * 1000);
          if (errorQueryAge <= 50000) {
            await Promise.race([
              ...
              new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Error answer timeout')), 15000)
              )
            ]);
          }
        } catch (answerError) {
          console.log(`⚠️ Could not send error response: ${answerError.message}`);
        }
        
        // 📊 Log error for monitoring
        this.logError('callback_query_error', error, {
          action,
          chatId,
          queryAge: Date.now() - (callbackQuery.message.date * 1000)
        });
      } finally {
  ...
  setupAxiosInterceptors() {
    ...
    // Response interceptor with metrics tracking
  ...
  // 💓 Enhanced heartbeat monitor with autonomous health checks
  setupHeartbeat() {
    console.log('💓 Setting up enhanced health monitoring system...');
    ...
    // Comprehensive health monitoring every 2 minutes
    this.heartbeatInterval = setInterval(async () => {
      try {
        await this.performHealthCheck();
      } catch (error) {
        console.error('💔 Health check error:', error);
        this.logError('health_check_error', error);
      }
  ...
  async performHealthCheck() {
    ...
    console.log(`🔍 Health check #${this.metrics.healthChecks} starting...`);
    
    // 1. Check if polling is active
    if (!this.isPollingActive) {
      console.log('⚠️ Polling inactive - attempting to restart');
      await this.restartBot();
      return;
    }
    
    // 2. Check bot connectivity with timeout
    try {
      const botInfo = await Promise.race([
        this.bot.getMe(),
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Bot getMe timeout')), 10000)
        )
      ]);
      
      this.lastHeartbeat = now;
      this.reconnectAttempts = 0;
      console.log(`💓 Bot connectivity OK - @${botInfo.username}`);
      
    } catch (error) {
      console.log(`💔 Bot connectivity failed: ${error.message}`);
      this.reconnectAttempts++;
      
      if (this.reconnectAttempts >= this.maxReconnectAttempts) {
        console.log('💔 Too many connectivity failures, restarting bot...');
        await this.restartBot();
        return;
      }
    }
    ...
    if (timeSinceLastHeartbeat > 300000) { // 5 minutes
      console.log(`💔 Heartbeat timeout (${(timeSinceLastHeartbeat/1000/60).toFixed(1)}m)`);
      await this.restartBot();
      return;
    }
    ...
    if (memUsageMB > 500) { // Alert if using > 500MB
  ...
  async performAutoRecovery() {
    try {
      ...
      if (this.errorLog.length > 50) {
        const removed = this.errorLog.splice(0, this.errorLog.length - 50);
        console.log(`🧹 Cleaned ${removed.length} old error entries`);
      }
      
      // Reset large counters
      if (this.metrics.messagesProcessed > 100000) {
        this.metrics.messagesProcessed = 0;
        this.metrics.callbacksProcessed = 0;
        this.metrics.apiCallsSuccess = 0;
        this.metrics.apiCallsFailure = 0;
        console.log('🔄 Reset processing counters');
      }
      
      // Check polling consistency
      if (this.isPollingActive) {
        try {
          await Promise.race([
            this.bot.getUpdates({ limit: 1, timeout: 1 }),
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('getUpdates timeout')), 5000)
            )
          ]);
        } catch (error) {
          if (error.message.includes('409') || error.message.includes('timeout')) {
            console.log('🔄 Detected polling inconsistency, fixing...');
            this.isPollingActive = false;
            await this.restartBot();
          }
        }
      ...
      console.log(`⚠️ Auto-recovery error: ${error.message}`);
    }
  }
  ...
  initProductionLogging() {
    ...
    console.log('🚀 === PRODUCTION BOT STARTUP ===');
    console.log(`📅 Start Time: ${new Date().toISOString()}`);
    console.log(`🌍 Environment: ${process.env.NODE_ENV || 'development'}`);
    console.log(`🏷️ Version: ${process.env.npm_package_version || 'unknown'}`);
  ...
  logMetrics() {
    ...
    console.log(`📨 Messages: ${this.metrics.messagesProcessed}`);
    console.log(`🔘 Callbacks: ${this.metrics.callbacksProcessed}`);
    console.log(`✅ API Success: ${this.metrics.apiCallsSuccess}`);
    console.log(`❌ API Failures: ${this.metrics.apiCallsFailure}`);
    console.log(`⚠️ Errors: ${this.metrics.errorsCount}`);
    console.log(`🔄 Restarts: ${this.metrics.restartCount}`);
    console.log(`❤️ Health Checks: ${this.metrics.healthChecks}`);
    console.log(`📡 Polling: ${this.isPollingActive ? '🟢 Active' : '🔴 Inactive'}`);
    console.log('==================');
  }
  ...
  async start() {
    try {
      ...
      this.initProductionLogging();

      // Start polling with enhanced stability and safety checks
      if (!this.isPollingActive) {
        try {
          // 🌐 Clear any existing webhooks to prevent 409 conflicts
          console.log('🧹 Preparing for polling by clearing webhooks...');
          await this.webhookManager.prepareForPolling();
        ...
        // Start polling with basic options (simplified for stability)
  ...
  setupForceStopCommand() {
    this.bot.onText(/\/force_stop/, async (msg) => {
      ...
      try {
        ...
        const forceStopResponse = await axios.post(`${this.baseUrl}/api/bot/stop`, {
  ...
  setupHealthCommand() {
    this.bot.onText(/\/health/, async (msg) => {
      ...
      try {
        ...
        const healthResponse = await axios.get(`${this.baseUrl}/api/bot/health`, {
          headers: {
            'x-bot-internal': 'true',
            'x-debug-skip-auth': 'true'
          },
