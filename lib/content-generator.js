...
TIME: ${this.formatTime(match.kickoffTime)}

Create ULTRA-CONCISE English prediction (max 2 lines):
1. ONE bet recommendation only (1X2, O/U, BTTS)
2. ONE sentence reasoning

Example: "Over 2.5 Goals - Both teams average 3+ goals in recent games"
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system", 
            content: "You are a professional sports betting analyst for GizeBets. Write ONLY in English. Ultra-concise predictions (max 2 lines). ONE bet only per match."
          },
          {
            role: "user",
            content: prompt
      ...
      // Clean format for Telegram
      content = this.cleanTelegramHTML(content);
      
      // Simple format: Just the prediction with match info
      const timeStr = this.formatTime(match.kickoffTime);
      const matchHeader = `⚽ ${match.homeTeam?.name || match.homeTeam} vs ${match.awayTeam?.name || match.awayTeam} (${timeStr})`;
...
Write ONE short exciting sentence about the most impressive result. Keep it brief and engaging.

Example: "Man City deliver stunning 4-goal masterclass"
`;
...
formatted += `\n💎 Promo_Code: ${promoCode}`;
  ...
  // Generate Daily Results in English - Top 5 only
  async generateDailyResults(results) {
    try {
      ...
      const topResultsList = scoredResults.map(result => 
        `${result.homeTeam} ${result.homeScore}-${result.awayScore} ${result.awayTeam} (${result.league || result.competition})`
      ).join('\n');

      const prompt = `
You are a sports results reporter for GizeBets Telegram channel.

Today's TOP Results (only the most important matches):
${topResultsList}

Create CONCISE English content with:

1. Brief title with emoji
2. Show results in clean format (3-4 lines max)
3. ONE short highlight about the best result
4. Keep it under 4 lines total
5. NO long commentary or analysis
...
🔥 Man City dominate with 4-goal thriller!
`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a concise sports reporter for GizeBets. Write ONLY in English. Keep messages SHORT (max 4 lines) for Telegram. Focus only on top-tier matches."
          },
          {
            role: "user",
            content: prompt
      ...
      // Clean up formatting for Telegram
      content = this.cleanTelegramHTML(content);
  ...
  // Clean up formatting for Telegram HTML
  cleanTelegramHTML(content) {
