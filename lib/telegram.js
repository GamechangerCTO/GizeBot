...
class TelegramManager {
  ...
  async sendPromo(content, promoCode) {
    return await this.retryRequest(async () => {
      ...
      // Send promo immediately, then upgrade with image in background
      console.log('🚀 Sending promo immediately...');
      ...
      // Generate AI image and UPDATE the message (background process)
      if (promoCode) {
        console.log('🎨 Starting background image upgrade...');
        
        // Run in background without blocking response
        this.upgradePromoWithImage(textMessage.message_id, formattedContent, keyboard, promoCode)
          .then(() => {
            console.log('✅ Promo successfully upgraded with image');
          })
          .catch(error => {
            console.log('⚠️ Background image upgrade failed:', error.message);
          });
      }
  ...
  // Upgrade text promo with AI image (background process)
  async upgradePromoWithImage(messageId, content, keyboard, promoCode) {
    try {
      ...
      const imageUrl = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Image generation timeout')), 50000) // 50 seconds
        )
      ]);

      if (imageUrl) {
        console.log(`🖼️ Image ready! Upgrading message ${messageId}...`);
        ...
        console.log(`✅ Promo upgraded! New Message ID: ${photoMessage.message_id}`);
        this.trackMessage('promo', photoMessage.message_id, { 
          promoCode, 
          hasImage: true, 
      ...
      console.log(`⚠️ Failed to upgrade message ${messageId}:`, error.message);
      // Original message stays as text, which is fine
    }
  }
