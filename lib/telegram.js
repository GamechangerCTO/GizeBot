...
class TelegramManager {
  ...
  // Send predictions as separate messages
  async sendPredictions(predictions) {
    ...
    try {
      // Send intro message if sending multiple predictions
      if (predictions.length > 1) {
        const introMessage = `🎯 <b>SENDING ${predictions.length} BETTING PREDICTIONS</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n📊 Each match analyzed separately for better focus`;
        
        await this.retryRequest(async () => {
          const introMsg = await this.bot.sendMessage(this.channelId, introMessage, {
            parse_mode: 'HTML',
            disable_web_page_preview: true
          });
          messageIds.push(introMsg.message_id);
        });
        
        // Small delay before predictions
      ...
      const totalMessages = predictions.length > 1 ? predictions.length + 1 : predictions.length; // +1 for intro message
      console.log(`✅ All ${predictions.length} predictions sent successfully (${totalMessages} total messages)`);
      return { messageIds, totalSent: predictions.length, totalMessages };
