...
class TelegramManager {
  ...
  formatSinglePredictionMessage(content) {
    ...
    lines.forEach((line) => {
      if ((line.includes('MATCH') && line.includes('/')) || (line.includes('LIVE MATCH') && line.includes('/'))) {
        ...
        updatedLine = updatedLine.replace('🔗 https://gizebets.et/', '⚽ כדורגל: https://gizebets.et/league?sportId=0\n🔴 משחקי כדורגל בזמן אמת: https://gizebets.et/live\n🎁 להזין קופון: https://gizebets.et/promo-campaigns\n💎 Promo_Code: gize251');
        formatted += `\n${updatedLine}\n`;
      } else if (line.trim()) {
        formatted += `${line}\n`;
      }
  ...
  formatResultsMessage(content) {
    ...
    lines.forEach(line => {
      if (line.includes(' - ') && (line.includes('1') || line.includes('2') || line.includes('3'))) {
        ...
        let updatedLine = line.replace(/gizebets\.et/g, 'https://gizebets.et/');
        updatedLine = updatedLine.replace('🔗 Full Analysis: https://gizebets.et', '⚽ כדורגל: https://gizebets.et/league?sportId=0\n🔴 משחקי כדורגל בזמן אמת: https://gizebets.et/live\n🎁 להזין קופון: https://gizebets.et/promo-campaigns\n💎 Promo_Code: gize251');
        formatted += `\n${updatedLine}\n`;
      } else if (line.trim()) {
        formatted += `${line}\n`;
      } else {
  ...
  formatPromoMessage(content, promoCode) {
    ...
    // Add striking header with emojis (no version info or separator lines)
    ...
    // Add call-to-action (no separator lines)
    formatted += `\n\n⚡ <b><i>🚀 CLAIM YOUR WINNING ADVANTAGE NOW! 🚀</i></b> ⚡\n` +
                 `🎯 <i>Join thousands of successful bettors!</i> 🎯`;
    
    // Remove website link from content (will be in buttons) and replace with proper links
