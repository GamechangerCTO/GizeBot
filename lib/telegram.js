...
class TelegramManager {
  ...
  async createPromoKeyboard(promoCode) {
    ...
    // Map configured buttons and de-duplicate any that look like another "Enter Coupon"
    const mapped = buttons
      .map(b => ({ text: b.text, url: this.createTrackingUrl(b.url, 'promo_btn') }))
      .filter(b => !/enter\s*coupon/i.test(b.text) && !/promo-campaigns/i.test(b.url));

    // Limit to 2 extra buttons so total is max 3
    const extra = mapped.slice(0, 2);

    // Build compact layout: up to 3 buttons total, no duplicates, no extra rows
    const rows = [];
    if (extra.length > 0) {
      rows.push([enterCouponBtn, extra[0]]);
    } else {
      rows.push([enterCouponBtn]);
    }
    if (extra.length > 1) {
      rows.push([extra[1]]);
    }
