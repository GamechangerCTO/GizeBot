...
class TelegramManager {
  ...
  async sendPromo(content, promoCode) {
    return await this.retryRequest(async () => {
      ...
      // Generate AI image for promo
      let imageUrl = null;
      if (promoCode) {
        console.log('🎁 Generating AI image for promo...');
        imageUrl = await this.imageGenerator.generatePromoImage(promoCode);
      }
  ...
  // Format promo message - clean and attractive
  formatPromoMessage(content, promoCode) {
    // Just enhance the AI-generated content without overwhelming it
    let formatted = content;
    
    // Ensure promo code is highlighted if mentioned
    ...
    // Make website link clickable and bold
    if (formatted.includes('🔗')) {
      formatted = formatted.replace(/🔗\s*(.+)/g, '<b>🔗 $1</b>');
    }
    
    // Bold any bonus percentages or amounts
    formatted = formatted.replace(/(\d+%\s*[Bb]onus)/g, '<b>$1</b>');
