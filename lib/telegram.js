...
class TelegramManager {
  ...
  // Format single prediction message (supports both regular and live formats)
  formatSinglePredictionMessage(content) {
    const lines = content.split('\n');
    let formatted = '';
    
    lines.forEach((line) => {
      if ((line.includes('MATCH') && line.includes('/5')) || (line.includes('LIVE MATCH') && line.includes('/3'))) {
        // Header line with match number and time (regular or live)
        formatted += `<b>${line}</b>\n`;
        formatted += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
      } else if (line.includes('vs') && line.includes('⚽')) {
        // Match teams
        formatted += `<b>${line}</b>\n`;
      } else if (line.includes('-') && !line.includes('━') && (line.includes('Manchester') || line.includes('Real') || line.includes('Arsenal') || line.includes('Chelsea'))) {
        // Live score line (e.g., "Manchester United 1-1 Liverpool")
        formatted += `<b>⚽ ${line}</b>\n`;
