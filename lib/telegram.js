...
class TelegramManager {
  ...
  async sendPromo(content, promoCode) {
    return await this.retryRequest(async () => {
      ...
      // Generate AI image FIRST (with short timeout for speed)
      let imageUrl = null;
      if (promoCode) {
        console.log('🎨 Generating AI image (20s timeout)...');
        try {
          const imagePromise = this.imageGenerator.generatePromoImage(promoCode);
          imageUrl = await Promise.race([
            imagePromise,
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Quick timeout - sending text')), 20000) // 20 seconds only
            )
          ]);
          console.log('✅ AI image ready!');
        } catch (error) {
          console.log('⚠️ Image timeout, sending text only:', error.message);
          imageUrl = null;
        }
      }

      // Send with image if ready, otherwise text only
      if (imageUrl) {
        ...
        console.log('✅ Promo with image sent, Message ID:', message.message_id);
        this.trackMessage('promo', message.message_id, { promoCode, hasImage: true });
        return message;
      } else {
        console.log('📝 Sending promo as text (no image)...');
        const message = await this.bot.sendMessage(this.channelId, formattedContent, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: keyboard
        ...
        console.log('✅ Promo text sent, Message ID:', message.message_id);
