...
class TelegramManager {
  constructor() {
    ...
    this.channelId = process.env.CHANNEL_ID; // e.g., @africansportdata
  ...
  async sendSummary(content) {
    return await this.retryRequest(async () => {
      const message = await this.bot.sendMessage(this.channelId, content, {
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        reply_markup: { inline_keyboard: this.createStandardKeyboard() }
      });
  ...
  async createPredictionsKeyboard() {
    return this.createStandardKeyboard();
  }
  ...
  createTrackingUrl(baseUrl, trackingId, options = {}) {
    ...
    const host = process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://sportmaster-bot.vercel.app');
    ...
    const channelDbUuid = options.channelId || process.env.CHANNEL_DB_UUID || process.env.SUPABASE_DEFAULT_CHANNEL_ID || '';
    if (channelDbUuid) redirect.searchParams.set('cid', channelDbUuid);
    if (options.appendUserId && options.userId) {
...
#GizeBets #Bonus #WinBig`;
  ...
  createPersonalCouponKeyboard(userId, promoCode) {
    ...
    return [
      ...
      [
        { text: '👤 Personal Codes', url: this.createTrackingUrl('https://t.me/Africansportbot', 'pc_personal_bot', { appendUserId: true, userId }) }
      ]
    ];
  }

  // Standard keyboard (applied to all messages): League, Enter Coupon, Personal Codes
  createStandardKeyboard() {
    return [
      [
        { text: '⚽ League', url: this.createTrackingUrl('https://gizebets.et/league', 'btn_league') },
        { text: '🎁 Enter Coupon', url: this.createTrackingUrl('https://gizebets.et/promo-campaigns', 'btn_coupon') }
      ],
      [
        { text: '👤 Personal Codes', url: this.createTrackingUrl('https://t.me/Africansportbot', 'btn_personal_codes') }
      ]
    ];
  }
