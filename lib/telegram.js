...
class TelegramManager {
  ...
  async sendPromo(content, promoCode) {
    return await this.retryRequest(async () => {
      ...
      // WAIT for AI image - NO PROMO WITHOUT IMAGE!
      if (!promoCode) {
        throw new Error('Promo code required for image generation');
      }

      console.log('🎨 Generating AI image (60s timeout - MUST have image)...');
      
      const imagePromise = this.imageGenerator.generatePromoImage(promoCode);
      const imageUrl = await Promise.race([
        imagePromise,
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Image generation failed after 60s - ABORTING PROMO')), 60000) // 60 seconds
        )
      ]);

      console.log('✅ AI image ready! Sending promo WITH image...');
      ...
      console.log('✅ Promo with image sent successfully, Message ID:', message.message_id);
