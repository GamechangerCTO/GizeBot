...
class TelegramManager {
  ...
  formatLiveStatusMessage(content) {
    ...
    lines.forEach(line => {
      if (line.startsWith('🔴 ') || line.startsWith('⚡ ')) {
        ...
        let updated = line.replace(/gizebets\.et/g, 'https://gizebets.et/');
        updated = updated.replace('https://gizebets.et/', '⚽ Football: https://gizebets.et/league?sportId=0\n🔴 Live Football: https://gizebets.et/live\n🎁 Enter Coupon: https://gizebets.et/promo-campaigns\n');
        formatted += `\n${updated}\n`;
      } else if (line.trim()) {
        formatted += `${line}\n`;
      }
  ...
  async createPredictionsKeyboard() {
    ...
    const row1 = filtered.slice(0, 2).map(b => ({ text: b.text, url: this.createTrackingUrl(b.url, 'predictions_btn') }));
    const rows = [];
    if (row1.length) rows.push(row1);
    if (filtered[2]) {
      rows.push([{ text: filtered[2].text, url: this.createTrackingUrl(filtered[2].url, 'predictions_btn3') }]);
    }
    // Always include coupon button with the ACTIVE code
  ...
  async createResultsKeyboard() {
    ...
    const row1 = filtered.slice(0, 2).map(b => ({ text: b.text, url: this.createTrackingUrl(b.url, 'results_btn') }));
    const rows = [];
    if (row1.length) rows.push(row1);
    if (filtered[2]) rows.push([{ text: filtered[2].text, url: this.createTrackingUrl(filtered[2].url, 'results_btn3') }]);
  ...
  async createPromoKeyboard(promoCode) {
    ...
    const enterCouponBtn = {
      text: `🎁 Enter Coupon ${promoCode}`,
      url: this.createTrackingUrl('https://gizebets.et/promo-campaigns', `promo_${promoCode}`)
    };
    ...
    const personalBtn = { text: '👤 Get Personal Coupons', url: 'https://t.me/Africansportbot?start=join_personal' };

    // Build rows: total max 3 buttons
    const rows = [[enterCouponBtn]];
    const secondRow = [];
    if (actionCandidate) {
      secondRow.push({ text: actionCandidate.text, url: this.createTrackingUrl(actionCandidate.url, 'promo_btn') });
    }
  ...
  createBonusKeyboard(bonusCode) {
    return [
      ...
      [
        { text: '👤 Get Personal Coupons', url: 'https://t.me/Africansportbot?start=join_personal' }
      ]
    ];
  }
  ...
  createPersonalCouponKeyboard(userId, promoCode) {
    ...
    return [
      [
        {
          text: `🎁 Enter Coupon ${promoCode}`,
          url: this.createTrackingUrl('https://gizebets.et/promo-campaigns', trackId, { appendUserId: true, userId })
        }
      ],
      [
        { text: '🔴 Live Football', url: this.createTrackingUrl('https://gizebets.et/live', 'pc_live', { appendUserId: true, userId }) }
      ]
    ];
  }
