...
class TelegramManager {
  ...
  async sendPromo(content, promoCode) {
    return await this.retryRequest(async () => {
      ...
      // Smart image strategy: Send text first, then update with image
      console.log('🚀 Sending promo text immediately...');
      
      // Send text message first (fast response)
      const textMessage = await this.bot.sendMessage(this.channelId, formattedContent, {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: keyboard
        },
      ...
      console.log('✅ Promo text sent, Message ID:', textMessage.message_id);
      this.trackMessage('promo', textMessage.message_id, { promoCode, hasImage: false });

      // Generate AI image in background (if promo code exists)
      if (promoCode) {
        console.log('🎁 Starting background AI image generation...');
        
        // Generate image without blocking the response
        this.generateAndUpdatePromoImage(textMessage.message_id, formattedContent, keyboard, promoCode)
          .catch(error => {
            console.log('⚠️ Background image generation failed:', error.message);
          });
      }
  ...
  // Generate and update promo with AI image (background process)
  async generateAndUpdatePromoImage(messageId, content, keyboard, promoCode) {
    try {
      console.log(`🎨 Generating AI image for message ${messageId}...`);
      
      // Generate image with extended timeout (60 seconds)
      const imagePromise = this.imageGenerator.generatePromoImage(promoCode);
      const imageUrl = await Promise.race([
        imagePromise,
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Image generation timeout after 60s')), 60000) // 60 seconds
        )
      ]);

      if (imageUrl) {
        console.log(`🖼️ Image generated, updating message ${messageId}...`);
        
        // Delete the text message
        await this.bot.deleteMessage(this.channelId, messageId);
        
        // Send new message with image
        const photoMessage = await this.bot.sendPhoto(this.channelId, imageUrl, {
          caption: content,
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: keyboard
        ...
        console.log(`✅ Promo updated with AI image, New Message ID: ${photoMessage.message_id}`);
        this.trackMessage('promo', photoMessage.message_id, { promoCode, hasImage: true, originalId: messageId });
      }
    } catch (error) {
      console.log(`⚠️ Failed to update message ${messageId} with image:`, error.message);
      // Message stays as text, which is fine
    }
  }
  ...
  formatPromoMessage(content, promoCode) {
    ...
    // Add striking header with emojis and visual separators
    formatted = `🚨 <b><u>🎯 VIP EXCLUSIVE ACCESS 🎯</u></b> 🚨\n` +
                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                `🔥 <i>V2.0 ENGLISH SYSTEM</i> 🔥\n\n` +
                `${formatted}`;
    ...
    // Enhanced text formatting for impact
    ...
    formatted = formatted.replace(/(limited|Limited|LIMITED|time)/gi, '<b><u>⏰ LIMITED TIME ⏰</u></b>');
    formatted = formatted.replace(/(VIP|vip|Vip)/g, '<b><i>👑 VIP 👑</i></b>');
    formatted = formatted.replace(/(win|Win|WIN|winning)/g, '<b>🏆 WIN 🏆</b>');
    
    // Add rich visual separators and call-to-action
    formatted += `\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                 `⚡ <b><i>🚀 CLAIM YOUR WINNING ADVANTAGE NOW! 🚀</i></b> ⚡\n` +
  ...
  createPromoKeyboard(promoCode) {
    return [
      ...
      [
        {
          text: '🚀 INSTANT REGISTER',
          url: this.createTrackingUrl('https://gizebets.et/register', 'promo_register')
        },
        {
          text: '💰 CLAIM BONUS',
          url: this.createTrackingUrl('https://gizebets.et/bonus', 'promo_claim')
        }
      ],
      [
        {
        ...
        {
          text: '🎯 LIVE BETTING',
          url: this.createTrackingUrl('https://gizebets.et/live', 'promo_live')
        }
      ],
      [
        {
        ...
        {
          text: '📞 VIP SUPPORT',
          url: this.createTrackingUrl('https://gizebets.et/support', 'promo_support')
        }
      ]
    ];
  }
  ...
  async executePromoCommand(promoType = 'football') {
    try {
      ...
      const promoConfigs = {
        football: {
          offer: '200% First Deposit Bonus + Free Bets',
          code: 'FOOTBALL200'
        },
        casino: {
          offer: '300% Casino Bonus + 100 Free Spins',
          code: 'CASINO300'
        },
        sports: {
          offer: '150% Sports Betting Bonus',
          code: 'SPORTS150'
        },
        special: {
          offer: '250% VIP Exclusive Bonus',
          code: 'VIP250'
        }
      };
