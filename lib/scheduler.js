// Scheduler for GizeBets Dynamic Automated Posts
...
class GizeBetsScheduler {
  ...
  async start() {
    ...
    console.log('🚀 Starting GizeBets Dynamic Scheduler...');
    this.isRunning = true;
  ...
  async initializeDynamicScheduling() {
    ...
    this.scheduleDynamicPromos();
    
    console.log('✅ Dynamic scheduling initialized');
  }

  // Load and analyze today's matches with enhanced data
  async loadTodayMatches() {
    try {
      console.log('📅 Loading today\'s enhanced matches...');
      
      // Try to get enhanced matches with detailed team data
      try {
        this.todayMatches = await this.footballAPI.getEnhancedTop5Matches();
        console.log('✅ Loaded enhanced match data with team statistics');
      } catch (enhancedError) {
        console.log('⚠️ Enhanced data failed, falling back to basic matches');
        this.todayMatches = await this.footballAPI.getTodayMatches();
      }
      ...
      console.log(`✅ Loaded ${this.todayMatches.length} matches for today`);
      ...
      // Log match quality
      const hasEnhancedData = this.todayMatches.length > 0 && this.todayMatches[0].homeTeamData;
      console.log(`📊 Match data quality: ${hasEnhancedData ? 'Enhanced (with team stats)' : 'Basic'}`);
  ...
  createDaySchedule(timings) {
    ...
    timings.allMatches.forEach((match, index) => {
      ...
      const predictionTime = new Date(match.kickoffTime.getTime() - (hoursBeforeMatch * 60 * 60 * 1000));
  ...
  async checkAndPostPredictions() {
    ...
    try {
      ...
      await this.telegram.sendPredictions(content);
  ...
  stop() {
    this.isRunning = false;
    console.log('🛑 GizeBets Scheduler stopped');
  }
  ...
  async sendScheduledPromo(timeSlot) {
    try {
      ...
      const promoCodes = {
        morning: { code: 'MORNING100', offer: '100% የጠዋት ቦናስ' },
        afternoon: { code: 'POWER200', offer: '200% አነጣጣሪ ቦናስ' },
        evening: { code: 'NIGHT250', offer: '250% የምሽት ቦናስ' }
      };
  ...
  async executeManualPredictions() {
    try {
      ...
      const matches = await this.footballAPI.getTodayMatches();
      if (matches.length === 0) {
        throw new Error('No matches available today');
      }
      
      const content = await this.contentGenerator.generateTop5Predictions(matches);
      const result = await this.telegram.sendPredictions(content);
  ...
  async executeManualResults() {
    try {
      ...
      const content = await this.contentGenerator.generateDailyResults(results);
      const result = await this.telegram.sendResults(content);
