...
class GizeBetsScheduler {
  ...
  async loadTodayMatches() {
    try {
      ...
      try {
        const { appendDailyMatches } = require('./storage');
        await appendDailyMatches(this.todayMatches);
      } catch (_) {}
  ...
  async executeManualResults() {
    try {
      ...
      let results = await this.footballAPI.getYesterdayResults();
      if (results.length === 0) {
        console.log('⚠️ No results from popular leagues. Falling back to ALL leagues...');
        const fallbackAll = await this.footballAPI.getAllYesterdayResults();
      ...
      // Prefer to use the matches we tracked today as the summary scope
      let scope = results;
      try {
        ...
        if (sched && Array.isArray(sched.matches) && sched.matches.length) {
          const names = new Set(sched.matches.map(m => `${m.homeTeam?.name||m.homeTeam}__${m.awayTeam?.name||m.awayTeam}`));
          const filtered = results.filter(r => names.has(`${r.homeTeam}__${r.awayTeam}`));
          if (filtered.length > 0) scope = filtered;
        }
      } catch (_) {}

      const content = await this.contentGenerator.generateDailyResults(scope);
      const result = await this.telegram.sendResults(content, scope);
