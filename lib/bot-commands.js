...
class GizeBotCommands {
  ...
  // 📋 Handle callback functions for menu panels - FAST version
  async handleManualPredictions(chatId, messageId) {
    // ⚡ IMMEDIATE response
    const response = this.bot.editMessageText(
    ...
    response.catch(error => console.error('❌ Error in handleManualPredictions:', error.message));
  }
  ...
  setupCallbackHandlers() {
    this.bot.on('callback_query', async (callbackQuery) => {
      ...
      try {
        // ⚡ IMMEDIATE response to remove loading spinner
        this.bot.answerCallbackQuery(callbackQuery.id).catch(() => {}); // Don't wait for this
        
        // ⚡ IMMEDIATE loading message update
        this.bot.editMessageText('⚡ Processing...', {
          chat_id: chatId,
          message_id: messageId
        }).catch(() => {}); // Don't wait for this either
        ...
        switch (action) {
          ...
          case 'manual_predictions':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleManualPredictions(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'manual_promo':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleManualPromo(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'live_matches':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleLiveMatches(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'upcoming_matches':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleUpcomingMatches(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'today_matches':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleTodayMatches(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'manual_results':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleManualResults(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'automation_panel':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleAutomationPanel(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'schedule_panel':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleSchedulePanel(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'settings_panel':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleSettingsPanel(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'analytics_panel':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleAnalyticsPanel(chatId, messageId).catch(console.error), 0);
            break;
          
          case 'coupons_panel':
            // ⚡ Immediate response with timeout
            setTimeout(() => this.handleCouponsPanel(chatId, messageId).catch(console.error), 0);
            break;
  ...
  async restartBot() {
    try {
      console.log('🔄 Restarting bot due to stability issues...');
      ...
      if (started) {
        ...
        await this.notifyAdmins('🔄 **BOT RESTARTED**', 
          `🔄 Bot was automatically restarted due to stability issues\n` +
