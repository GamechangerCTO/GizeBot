  ...
  async start() {
    try {
      ...
      if (!this.isPollingActive) {
        ...
        // Start polling with enhanced options for stability
        await this.bot.startPolling({
          restart: false, // Disable auto-restart to control it manually
          onlyFirstMatch: true, // Process only first match for commands
          interval: 3000, // Increase polling interval to reduce load (3 seconds)
          limit: 30 // Reduce updates per request to prevent overload
        });
