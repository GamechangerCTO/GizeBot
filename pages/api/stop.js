// API Endpoint to stop the GizeBets automated system
...
export default async function handler(req, res) {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'GizeBets automated system stopped successfully',
    ...
    console.error('❌ Error stopping GizeBets system:', error);
    
    res.status(500).json({
      success: false,
      message: 'Failed to stop GizeBets system',
      error: error.message,
